...
class WorkflowTemplateList {
  ...
  _renderTemplateCard(template) {
    ...
    // KudoFlow99: AI provider tags from template nodes
    const _nodes = template.nodes || [];
    const _providerTags = [];
    if (_nodes.some(n => n.class === 'generate' || n.data?.node_type === 'generate')) _providerTags.push({ label: 'Google Flow', color: '#3186FF' });
    if (_nodes.some(n => n.class === 'chatgpt'  || n.data?.node_type === 'chatgpt'))  _providerTags.push({ label: 'ChatGPT',    color: '#10A37F' });
    if (_nodes.some(n => n.class === 'grok'     || n.data?.node_type === 'grok'))     _providerTags.push({ label: 'Grok',       color: '#a855f7' });
    const providerTagsHtml = _providerTags.length > 0
      ? `<div class="wf-template-ai-tags">${_providerTags.slice(0, 2).map(tag => `<span class="wf-template-ai-tag" style="--tag-color:${tag.color}">${tag.label}</span>`).join('')}</div>`
      : '';
