...
class DiagramCanvas {
  ...
  bindEvents() {
    ...
    // KudoFlow99: Connection hover tooltip — shows data type on connection hover
    const _connTypeLabels = { text: 'Văn bản', image: 'Hình ảnh', video: 'Video', frame: 'Frame', audio: 'Audio' };
    const _connTooltip = this.container.querySelector('#connTooltip');
    const _canvasEl = this.container.querySelector('.diagram-canvas');
    _canvasEl?.addEventListener('mousemove', (e) => {
      if (!_connTooltip) return;
      const path = e.target.closest('svg.connection .main-path');
      if (!path) { _connTooltip.style.display = 'none'; return; }
      const svgConn = path.closest('svg.connection');
      const connType = [...(svgConn?.classList || [])].find(c => c.startsWith('conn-type-'))?.replace('conn-type-', '') || 'text';
      _connTooltip.textContent = `↔  ${_connTypeLabels[connType] || connType}`;
      const rect = this.container.getBoundingClientRect();
      _connTooltip.style.left = (e.clientX - rect.left + 14) + 'px';
      _connTooltip.style.top = (e.clientY - rect.top - 34) + 'px';
      _connTooltip.style.display = 'block';
    });
    _canvasEl?.addEventListener('mouseleave', () => { if (_connTooltip) _connTooltip.style.display = 'none'; });
